# @jkarki, trying to change the dataloader to not load everything at once.
...
def get_train_dataloader(train_path, crop_size=128, batch_size=1, downscale=0, concat=True, scale_min=0.001, scale_max=0.1, grey=False):
	if downscale == 0:
	...
	train_dataset = MLRDataset(train_path, train_transforms, grey=grey)
	
	return torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, num_workers=3, drop_last=True, shuffle=True)
